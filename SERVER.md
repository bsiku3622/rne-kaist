...
- **프로젝트 루트**: `D:\School\KSA\RnE\rne-kaist`
...
```powershell
$root = "D:\School\KSA\RnE\rne-kaist"
Set-Location $root
...
터미널에서 `tensorboard --logdir archive &` 처럼 그냥 백그라운드로 돌리면 **셸 세션이 끝날 때 같이 죽습니다.** 실제로 아무 오류 로그도 남기지 않고 조용히 종료됩니다. `Start-Process`는 셸에서 분리된 독립 프로세스로 띄우므로 터미널을 닫아도 살아 있습니다.
