"""Reorder the shared corpus into the ``(t, z, y, x)`` columns CMLP expects.

:mod:`models.mlp.dataset` with the power column dropped on the way through.
:class:`~dataset.SimulationDataset` hands out ``coords[N,4]`` as ``(x, y, z, t)``
alongside ``power[N,1]``; the network wants the spatial axes reversed and the
power not at all, so the transposition lives here rather than being repeated at
every call site.

``power`` is still *drawn* -- the batches come from the same rows ``mlp`` trains
on, so the two see an identical stream of labels -- it is simply not passed on.
"""
...
class CMLPDataset:
    ...
    def normalisation(self) -> tuple[list[float], list[float]]:
        ...
        # (t, z, y, x). `mlp` normalises a fifth column, P, from zero; there is
        # no such column here.
