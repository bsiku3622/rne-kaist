"""A physics-informed DeepONet with the gaussian gate. The upstream flagship.

Ported from ``typeulli-model-training/models/gpidon``. Its ``model.py``, ``loss.py``,
...
def main(argv: list[str] | None = None) -> None:
    ...
    sampler = GPiDoNDataset(s.train, s.generator)
    architecture = dict(
        branch_input_dim=1,
    ...
    model = s.to(GPiDoN(**architecture))
    ...
    harness.go(NAME, s, model, architecture, GPiDoNAgent, step,
