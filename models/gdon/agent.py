"""GDoN behind the shared :class:`~agent.BaseAgent` inference contract.

``predict_at`` is a direct forward pass -- the network is already a pointwise
map ``(x, y, z, t; P) -> T``, so no interpolation is involved at any query point
and the volumetric ``predict_of`` inherited from the base class is just the same
map evaluated on a regular grid.
"""
...
class GDoNAgent(BaseAgent):
    """Wraps a trained :class:`~models.gdon.model.GDoN` for inference."""
...
def build_agent(
    ...
    model = GDoN(**payload["architecture"])
    model.load_state_dict(payload["model"])  # normalisation buffers ride along
    return GDoNAgent(model, payload["bounds"], shape=shape, device=device)
