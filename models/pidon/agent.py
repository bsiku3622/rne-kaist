"""PiDoN behind the shared :class:`~agent.BaseAgent` inference contract.
...
class PiDoNAgent(BaseAgent):
    """Wraps a trained :class:`~models.pidon.model.PiDoN` for inference."""
...
def build_agent(
    ...
    model = PiDoN(**payload["architecture"])
    model.load_state_dict(payload["model"])  # normalisation buffers ride along
    return PiDoNAgent(model, payload["bounds"], shape=shape, device=device)
