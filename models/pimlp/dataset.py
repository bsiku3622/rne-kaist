...
normal attached, points at ``t = 0``. They are sampled here from the same domain
the labels span.
...
def normalisation(domain: Domain, max_power: float) -> tuple[list[float], list[float]]:
    """``(mean, scale)`` per column of ``(P, t, z, y, x)``, taken from the corpus bounds.
    ...
    # P is scaled from zero because a laser power of zero is physically
    # meaningful (no heating) while its centre is an artefact of whichever
    # powers happen to be in the sweep.
...
class PiMLPDataset:
    """Samplers for every term of :class:`~models.pimlp.loss.PINNLoss`.
    ...
    def top(self, count: int) -> PointSet:
        ...
        normal[:, 2] = 1.0
        flux = laser_flux(coords, power, exponent_scale=self.gaussian_exponent_scale)
        return PointSet(
            laser_power=power,
