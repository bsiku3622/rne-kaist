"""PiMLP behind the shared :class:`~agent.BaseAgent` inference contract.

``predict_at`` is a direct forward pass -- the network is already a pointwise map
``(x, y, z, t; P) -> T``, so no interpolation is involved at any query point and
the volumetric ``predict_of`` inherited from the base class is just the same map
evaluated on a regular grid. The ``(x, y, z, t, P)`` of the contract splits
straight into the ``(laser_power, coords)`` the model takes; the reordering into
``(P, t, z, y, x)`` happens inside :meth:`~models.pimlp.model.PhysicsMLP.forward`.
"""
...
class PiMLPAgent(BaseAgent):
    """Wraps a trained :class:`~models.pimlp.model.PhysicsMLP` for inference."""
...
def build_agent(
    ...
    model = PhysicsMLP(**payload["architecture"])
    model.load_state_dict(payload["model"])  # normalisation buffers ride along
    return PiMLPAgent(model, payload["bounds"], shape=shape, device=device)
