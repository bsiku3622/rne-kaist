"""GMLP behind the shared :class:`~agent.BaseAgent` inference contract.

As in :mod:`models.mlp.agent`, the only work ``predict_at`` does beyond a forward
pass is reordering the columns: the contract is ``(x, y, z, t, P)`` and the
network takes ``(P, t, z, y, x)``. The gate is applied inside the network, from
those same columns, so nothing here has to know about the beam.
"""
...
class GMLPAgent(BaseAgent):
    """Wraps a trained :class:`~models.gmlp.model.GatedMLP` for inference."""
...
def build_agent(
    ...
    model = GatedMLP(**payload["architecture"])
    model.load_state_dict(payload["model"])  # normalisation buffers ride along
    return GMLPAgent(model, payload["bounds"], shape=shape, device=device)
