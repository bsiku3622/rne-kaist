"""Reorder the shared corpus into the ``(P, t, z, y, x)`` columns GMLP expects.

A copy of :mod:`models.mlp.dataset`: the gate lives inside the network and is
...
as ``(x, y, z, t)`` alongside ``power[N,1]``; the network wants them in one
tensor, process parameter first and spatial axes reversed.
"""
